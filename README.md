...
- **Explainable AI (XAI)**: No "Black Box" metrics. The dashboard provides a human-readable **"Why?"** for every anomaly inline.
...
## 🚀 Key Features
- **Real-time Performance Metrics**: Zero-overhead CPU/Memory tracking via direct Kernel parsing.
- **Hero Terminal**: Full interactive shell terminal integrated into the main dashboard for real-time command execution.
- **Headless WiFi Module**: Hardware-level scanning via `iw` and robust joining via `wpa_cli`.
- **Enhanced File Explorer**: Full access to the host filesystem (`/host`) with built-in text editor and fallback for local development.
- **Advanced Battery Hub**: Deep battery analytics for edge devices with multi-path Linux support.
- **Unified Process Tracker**: Single-view management of all active system processes with resource delta tracking.
...
- **RAM Idle**: ~18.9 MiB (Goal: <20 MiB)
- **Docker Image**: 155 MB (Goal: <80 MiB)
